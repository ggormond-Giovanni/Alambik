...
static func depuis_reglages(rangs: Dictionary = {}, bonus_niveau_pv := 0.0,
		mult_niveau_degats := 1.0, mult_niveau_vitesse := 1.0, passifs: Dictionary = {}, objets: Dictionary = {}) -> Stats:
	var s := Stats.new()
	s.pv_max = (Reglages.HEROS_PV + bonus_niveau_pv + ArbreCompetences.bonus_pv(rangs) + float(objets.get("pv", 0.0))) * Sorts.multiplicateur_pv(passifs)
	s.pv = s.pv_max
	s.vitesse = Reglages.HEROS_VITESSE * mult_niveau_vitesse * ArbreCompetences.multiplicateur_vitesse(rangs) * Sorts.multiplicateur_vitesse(passifs) * (1.0 + float(objets.get("vitesse", 0.0)))
	...
	s.degats = Reglages.TIR_DEGATS * mult_niveau_degats * ArbreCompetences.multiplicateur_degats(rangs) * Sorts.multiplicateur_degats(passifs) * (1.0 + float(objets.get("degats", 0.0)))
